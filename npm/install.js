...
function getPlatformInfo() {
  ...
  return {
    ...
    url: `https://github.com/blankeos/modelcli/releases/download/v${VERSION}/${BINARY_NAME}-${target}${extension}`,
  };
}
...
function logInstallFailure(error) {
  ...
  console.error("\nYou can install modelcli directly using:");
  console.error(
    'curl --proto "=https" --tlsv1.2 -LsSf https://github.com/blankeos/modelcli/releases/latest/download/modelcli-installer.sh | sh',
  );
}
...
async function install({ exitOnComplete = false } = {}) {
  try {
    ...
    console.log(`✅ modelcli v${VERSION} installed successfully!`);
